...
//using arrow key to move through coordinates
void gotoxy(int x,int y)    
{
    printf("%c[%d;%df",0x1B,y,x);//curser position
}
...
//dynamic display when user navigates
...
// The main function that handles user interaction and highlights options
int WhiteHighlight(char header[], char options[][20], int NumberOfOptions, int initialOptionHighlight)
{
    ...
    do
    {
        ...
        if(ch==75 || ch==72)    //Left and up
        {
            if(NewChoice==0)
                continue;
            NewChoice--;
        }
        else if(ch==77 || ch==80)  //right down
        {
